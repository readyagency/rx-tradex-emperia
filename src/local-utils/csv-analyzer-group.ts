...
const getEmailConfirmUrl = async (data: DataStructure[]) => {
    ...
    for (const value of data) {
        const { title, fullName, companyName, email, badgeId, groupId } = value;
        if (groupId !== badgeId) {
            results += "\n";
            continue;
        }
    ...
    fs.writeFile("src/csvs/email-confirm-group.txt", results, error => {
...
const getEmailRecommendUrl = async (data: DataStructure[]) => {
    ...
    fs.writeFile("src/csvs/email-recommend-group.txt", results, error => {
        if (error) console.log("error::", error);
    });
};
...
const getEncryptKeyUrl = async (data: DataStructure[]) => {
    ...
    fs.writeFile("src/csvs/encrypt-code-group.txt", results, error => {
        if (error) console.log("error::", error);
    });
};
...
const analyzeData = () => {
    const fileContent = fs.readFileSync(
        "src/csvs/Collection Data - Group.json",
        { encoding: "utf-8" },
    );
...
convertCSVToArray("src/csvs/Collection Data - Group.csv");
analyzeData();
